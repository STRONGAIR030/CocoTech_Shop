...
const CheckOutPage = () => {
	// setshowCreditCard 應改為 setShowCreditCard.
	const [showCreditCardInf, setshowCreditCard] = useState(false);
	const [errorText, setError] = useState('');
	const [errorKey, setKey] = useState(uuidv4());
	const { shoppingCart, modifyProductToCart, userInf, clearShoppingCart } =
	...
	const totalCost = useMemo(() => {
		return shoppingCart.reduce(
			(prevValue, product) => product.totalPrice + prevValue,
			0
		);
	}, [shoppingCart]);
	// ShippingCost 應為 shippingCost（駝峰式命名）
	...
	// dateFormat 命名過於模糊，建議改為 formatDate。
	...
	// handleSubmit 責任過多，包含多層資料檢查、表單驗證與 API 請求
	// 將邏輯拆分為小型函式，例如：
	// 	1.	表單驗證邏輯
	// 	2.	資料建構
	// 	3.	API 提交
	const handleSubmit = async (e) => {
		...
		const NecCreditData = ['cardName', 'cardNum', 'cardCode', 'cardDate'];
		// 多次出現 formData[name] && formData[name] != ""。
		// 使用工具函式簡化
	...
	// JSX 結構過於深，增加維護難度，應提取為子元件，提升模組化。
	return (
		<CheckOutLayout>
			<StyledCheckOutPage>
				<ListSwitch
					mdShow
					handleClick={() => {
						setShowOrderSummary((prevShow) => !prevShow);
					}}
				...
				<StyledUserInf>
					<SyledOrderForm onSubmit={handleSubmit}>
						...
						<UserRadio
							radioText="FamilyMart 60$"
							inputName="shipping"
							inputValue="familyMart"
							handleChange={handleChange}
						/>
						<UserRadio
							radioText="7-11 60$"
						...
						<UserRadio
							radioText="Cash on delievery"
							inputName="payment"
							inputValue="cashOnDelievery"
							handleChange={handleChange}
						/>
						{errorText && (
