...
function FrontEndApp() {
    ...
    useEffect(() => {
        const updateShoppingCart = async () => {
            try{
                ...
                await axios.put(`${API_HOST}/shoppingCart/${userInf.id}`, {cartContent: formattedCurrentCart})
            } catch (err) {
    ...
    const modifyProductToCart = (productId, quantity, name, imgUrl, price) => {
        ...
        setCart((prevCart) => {
            if(prevCart.some(product => product.id == productId)){
                if(quantity === ACTION_DELETE || prevCart.some(product => product.id == productId && product.quantity + quantity <= 0)){
    ...
    const createShoppinCart = async (userId) => {
        try {
            ...
            const postResponce = await axios.post(`${API_HOST}/shoppingCart`, newCart)
    ...
    const syncShoppingCart = async (userId) => {
        try{
            const {data : cartData} = await axios.get(`${API_HOST}/shoppingCart/${userId}`)
            console.log(cartData);
             
            const formattedCartData = await Promise.all(cartData.cartContent.map(async (product) => {
